...
bkcore.hexgl.ShipControls = function (domElement) {
    ...
    this.middleCheckVLeft = new THREE.Vector3(1, 0, 0);
    this.middleCheckVRight = new THREE.Vector3(-1, 0, 0);
    this.CollisionDebug = "";
    this.isMiddle = false;
    this.middleScale = 1;
    this.middleTime = 0;
    ...
    function onKeyDown(event) {
        switch (event.keyCode) {
            ...
            case 90: /*Z*/self.key.bonus = true; break;
        }
    };
    ...
    function onKeyUp(event) {
        switch (event.keyCode) {
            ...
            case 90: /*Z*/self.key.bonus = false; break;
        }
    };
...
bkcore.hexgl.ShipControls.prototype.reset = function(position, rotation)
{
	...
	this.speedRatio = 0.0;
	this.boost = 0.0;
	this.shield =0.0;// this.maxShield;
	this.destroyed = false;
...
bkcore.hexgl.ShipControls.prototype.update = function (dt) {
   ...
   // console.log(this.elapsedTime + " " + this.getLevelTime());
    if (this.elapsedTime >= this.getLevelTime()) {
        this.destroy();
...
bkcore.hexgl.ShipControls.prototype.collisionCheck = function (dt) {
    ...
    this.middleScale = 6;
    this.isMiddle = 0;
    this.middleCheckVLeft.set(1, 0, 0);
    this.middleCheckVRight.set(-1, 0, 0);
    this.dummy.matrix.rotateAxis(this.middleCheckVLeft);
    this.dummy.matrix.rotateAxis(this.middleCheckVRight);
    this.middleCheckVLeft.multiplyScalar(this.middleScale);
    this.middleCheckVRight.multiplyScalar(this.middleScale);

    var lmiddlePos = this.middleCheckVLeft.addSelf(pos);
    var rmiddlePos = this.middleCheckVRight.addSelf(pos);
    var lmiddleCol = this.collisionMap.getPixel(Math.round(lmiddlePos.x), Math.round(lmiddlePos.z)).r;
    var rmiddleCol = this.collisionMap.getPixel(Math.round(rmiddlePos.x), Math.round(rmiddlePos.z)).r;

    if (this.resetMiddleTime == false) {
    ...
    if (lmiddleCol == 0) {
        this.CollisionDebug = "Go Right";
        this.shield = 0;
        if (this.resetMiddleTime == false)
            this.middleTime = 0;
    }
    else if (rmiddleCol == 0) {
        this.CollisionDebug = "Go Left";
        this.shield = 0;
        if (this.resetMiddleTime == false)
            this.middleTime = 0;
    }
...
bkcore.hexgl.ShipControls.prototype.getMiddleTotalTime = function()
{
	return (this.middleTotalTime < 0.02 ? 0:this.middleTotalTime);
};

bkcore.hexgl.ShipControls.prototype.getLevelTime = function()
{
